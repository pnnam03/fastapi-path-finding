...
def a_star(start, stop, NODES_FILE, EDGES_FILE):
    ...
    f_score = {start: cost(start, stop)}

    # ý tưởng của A* same same Dijikstra, với nút kề với nút đang xét và không thuộc before thì chọn nút có g_score +
    # f_score min, sau đó update các nút khác
    while open_list:
        ...
        for neighbour in graph[current]:
            ...
            if neighbour not in g_score or current_g_score < g_score[neighbour]:
                ...
                f_score[neighbour] = current_g_score + cost(neighbour, stop)
